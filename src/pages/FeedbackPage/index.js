...
export default function FeedbackPage({ feedback, playAgain=() => {} }) {
    ...
    return (
        <>
            <FeedbackAlert variant={(result?"success":"danger")} onClick={playAgain}>
                {`${result?"Correct answer :)":"Wrong answer"}`}
                <br/>
                {
                    correct_answer_formatted.length > 1?
                    correct_answer_formatted.map(
                        (item, index) => ((index === correct_answer_formatted.length - 1)?
